...
const icons = [Users, User, Monitor, Briefcase];
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10 lg:px-14">
        <div className="grid grid-cols-1 md:grid-cols-12 items-end gap-8 mb-14 md:mb-20">
          <div className="md:col-span-8">
            ...
            <h2 className="font-serif text-5xl md:text-6xl lg:text-7xl leading-[0.95] mt-5 text-ink">
              {s.h2a}
              <br />
              <span className="italic">{s.h2b}</span>
            </h2>
          </div>
          ...
          {s.classes.map((c, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`relative bg-paper-warm border border-line p-8 md:p-10 hover:bg-ink hover:text-paper transition-all duration-500 group ${spans[idx]}`}
              >
                <Icon className="w-7 h-7 text-brand group-hover:text-paper transition-colors duration-500" strokeWidth={1.4} />
                <h3 className="font-serif text-3xl md:text-4xl mt-6 leading-tight">{c.title}</h3>
        ...
        <div className="mt-16 md:mt-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-line border border-line">
            {s.levels.map((lvl, idx) => (
              ...
              >
                <div>
                  <div className="font-serif text-3xl md:text-4xl text-ink">{lvl.name}</div>
                  <div className="font-sans text-xs uppercase tracking-[0.2em] text-ink-muted mt-2">{lvl.small}</div>
                </div>
        ...
        >
          <p className="font-serif text-3xl md:text-4xl text-ink max-w-xl leading-snug">
            {s.ctaText}{" "}
            <span className="italic text-brand">{s.ctaItalic}</span>
          </p>
          <a
            href={PHONE_LINK}
